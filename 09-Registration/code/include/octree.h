...
bool octree_radius_search_fast(octant * root, Eigen::MatrixXd db, RadiusNNResultSet &result_set, Eigen::Matrix<double, 1, 3> query){
    ...
    if (contains(query, result_set.get_worstDist(), root)){
        //比较当前网格中的所有点
...
//        }
    ...
    if (root->is_leaf && (root->point_indices.size() > 0)){
        ...
        return inside(query, result_set.get_worstDist(), root);
    }
    ...
    for (int i = 0; i < root->children.size(); ++i) {
        ...
        //如果搜索点的球体和正方体不相交
        if (false == overlaps(query, result_set.get_worstDist(), root->children[i])){
            continue;
        }
        //递归搜索子节点
