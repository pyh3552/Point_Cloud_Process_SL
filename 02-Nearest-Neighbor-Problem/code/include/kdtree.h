...
public:
    ...
    //插入点
    void add_point(kdtree *tree, double dist, size_t index){
...
//  result_set:搜索结果
//  query：要搜索近邻的目标对象
bool kdtree_knn_search(kdtree *tree, Eigen::MatrixXd pointset, KnnResultSet &resultset, Eigen::Vector3d query){
//    cout << "递归了一次" << endl;
    if (tree->is_empty())
        return false;
...
//    int i = 0;
    if (!tree->is_empty() ){
//        k : tree->node;
        Eigen::Vector3d leaf_points;
        leaf_points[0] = tree->node[0];
        leaf_points[1] = tree->node[1];
        leaf_points[2] = tree->node[2];
//        cout << "leaf_points: " << leaf_points;
        double dist = (query - leaf_points).norm();
        for (int i = 0; i < pointset.rows(); ++i) {
            if (pointset(i,0)==leaf_points[0] && pointset(i,1)==leaf_points[1] && pointset(i,2)==leaf_points[2]){
                tree->point_indices = i;
            }
        }
        resultset.add_point(tree, dist, tree->point_indices);
//        i++;
//        return false;
    }
